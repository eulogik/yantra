...
def evaluate_case(llm, prompt, case):
    resp = llm(prompt, temperature=0, max_tokens=768, stop=["\n<bind", "<tool_result>", "<user>"])
...
if prog.exists():
    ...
    if first.get("git","") != "router-v2-raw":
...
for i, case in enumerate(cases):
    ...
    rec = {"i": i, "git": "router-v2-raw", **b, **meta}
    per.append(rec)
...
out = {"PAS": round(pas(agg),4), **agg, "per_case": per, "git": "router-v2-raw", "model": str(gguf)}
if res_file.exists():
